...
const PersonalNotification = () => {
  ...
  useEffect(() => {
    const getNotiList = async () => {
      ...
      const response = await getApi(
        `/notificationList?notiCode=${searchCondition.notiCode}&startDate=${searchCondition.startDate}&endDate=${searchCondition.endDate}&content=${searchCondition.content}&size=${searchCondition.size}&page=${searchCondition.page}`
      );
  ...
  return (
    <>
      ...
      {!isLoading && (
        <>
          <NotificationList notiPageList={notificationPageList} ON />
          <Pagination
